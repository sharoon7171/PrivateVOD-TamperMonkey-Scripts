// ==UserScript==
// @name         PrivateVOD Hide Liked Videos
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Hides grid items containing ONLY liked (not favourited) scene IDs from localStorage with real-time DOM manipulation
// @author       SQ Tech
// @homepageURL  https://sqtech.dev
// @updateURL    https://raw.githubusercontent.com/sharoon7171/PrivateVOD-TamperMonkey-Scripts/main/PrivateVOD%20Hide%20Liked%20Videos/privatevod%20hide%20liked%20videos.user.js
// @downloadURL  https://raw.githubusercontent.com/sharoon7171/PrivateVOD-TamperMonkey-Scripts/main/PrivateVOD%20Hide%20Liked%20Videos/privatevod%20hide%20liked%20videos.user.js
...
(function() {
    ...
    console.log('👍 PrivateVOD Hide Liked Videos script loaded');
    
    // Configuration
    const CONFIG = {
        favouritesStorageKey: 'privatevod_favourites',
        likesStorageKey: 'privatevod_likes',
        hideClass: 'privatevod-hidden-liked',
        debugMode: false
    };
    ...
    function getFavouritedScenes() {
        try {
            const stored = localStorage.getItem(CONFIG.favouritesStorageKey);
    ...
    // Get liked scenes from localStorage
    function getLikedScenes() {
        try {
            const stored = localStorage.getItem(CONFIG.likesStorageKey);
            return stored ? new Set(JSON.parse(stored)) : new Set();
        } catch (e) {
    ...
    // Process all grid items and hide/show based on likes (but not favourites)
    function processGridItems() {
        ...
        if (CONFIG.debugMode) {
            ...
            console.log(`👍 Liked scenes: [${Array.from(likedScenes).join(', ')}]`);
        }
        ...
        gridItems.forEach((gridItem, index) => {
            ...
            if (sceneId) {
                ...
                const isLiked = likedScenes.has(sceneId);
                
                // Hide only if liked BUT NOT favourited
                if (isLiked && !isFavourited) {
                    if (hideGridItem(gridItem)) {
                        hiddenCount++;
                        if (CONFIG.debugMode) {
                            console.log(`🚫 Hidden liked-only scene ${sceneId} (item ${index + 1})`);
                        }
                    }
                } else {
                    // Show if not liked, or if both liked and favourited
                    if (showGridItem(gridItem)) {
                        shownCount++;
                        if (CONFIG.debugMode) {
                            console.log(`👁️ Shown scene ${sceneId} (item ${index + 1}) - ${isFavourited ? 'favourited' : 'not favourited'}, ${isLiked ? 'liked' : 'not liked'}`);
                        }
                    }
                }
        ...
        if (hiddenCount > 0 || shownCount > 0) {
            console.log(`✅ Processed grid items: ${hiddenCount} hidden (liked-only), ${shownCount} shown`);
        }
    ...
    function setupStorageMonitor() {
        ...
        window.addEventListener('storage', (e) => {
            if (e.key === CONFIG.favouritesStorageKey || e.key === CONFIG.likesStorageKey) {
                console.log('🔄 Favourites or likes storage changed, updating visibility');
                processGridItems();
            }
        });
        ...
        localStorage.setItem = function(key, value) {
            ...
            if (key === CONFIG.favouritesStorageKey || key === CONFIG.likesStorageKey) {
                console.log('🔄 Favourites or likes storage updated, updating visibility');
    ...
    function init() {
        ...
        console.log('🚀 PrivateVOD Hide Liked Videos ready');
        console.log('💡 Use PrivateVODHideLiked.process() to manually process');
    }
